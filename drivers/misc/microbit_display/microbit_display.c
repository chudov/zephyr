...
static const uint32_t col_mask0 =
	1 << DT_GPIO_PIN(DT_NODELABEL(col_1), gpios) |
...
static const uint32_t row_mask =
	1 << DT_GPIO_PIN(DT_NODELABEL(row_1), gpios) |
	...
	for (int x = 0; x < DISPLAY_COLS; x++) {
		...
		for (int y = 0; y < DISPLAY_ROWS; y++) {
			if (((offset < DISPLAY_COLS)
				  && ((img1->row[y] >> offset) & 1)) ||
				((offset >= SYMBOL_WIDTH)
				  && ((img2->row[y] >> (offset - SYMBOL_WIDTH)) & 1))
			   ) {
...
static void init_display_timer(void)
{
	...
	/* By default 2500 Hz pixel frequency */
	display_timer.p_reg->CC[0] = 62500 / (DISPLAY_ROWS * DISPLAY_COLS
						* CONFIG_MICROBIT_DISPLAY_REFRESH_RATE_HZ);
